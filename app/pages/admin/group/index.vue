    ...
    <Dialog v-model:open="studentsDialogOpen">
      <DialogContent class="max-w-3xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          >
          <DialogDescription>
            {{ currentGroup?.student_ids?.length || 0 }} /
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-2">
            ...
            >
              <div
                v-for="studentId in currentGroup?.student_ids"
                :key="studentId"
                class="flex items-center justify-between p-2 hover:bg-muted rounded-md"
              >
                <div class="flex items-center gap-3">
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  @click="removeStudentFromGroup(studentId)"
                >
                  <X class="h-4 w-4" />
                </Button>
              </div>
              <div
                v-if="!currentGroup?.student_ids?.length"
                class="text-center py-4 text-muted-foreground"
              >
...
const filteredAvailableStudents = computed(() => {
  ...
  const enrolledIds = currentGroup.value?.student_ids || [];
...
const isGroupFull = computed(() => {
  ...
  return (
    (currentGroup.value.student_ids?.length || 0) >=
...
const saveGroup = async () => {
  try {
    const payload = {
      ...
      student_ids: isEditing.value ? currentGroup.value.student_ids : [],
    };
...
const addStudentToGroup = async (studentId: string) => {
  ...
  try {
    ...
    // Update local state
    if (!currentGroup.value.student_ids) {
      currentGroup.value.student_ids = [];
    }
    currentGroup.value.student_ids.push(studentId);
...
const removeStudentFromGroup = async (studentId: string) => {
  ...
  try {
    ...
    // Update local state
    currentGroup.value.student_ids = currentGroup.value.student_ids.filter(
